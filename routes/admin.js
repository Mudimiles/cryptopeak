...
router.post('/admin/admin.investmentplans/crypto-mining',isAdminLoggedIn, onlyAdmin, async(req, res) => {
    const id  = req.user.id;
    const admin = await Users.findById(id);
    const {packagename, amount, duration, profitpercentage, charges, power, storage} = req.body;
    const investmentplans = new InvestmentPlans({packagename, amount, duration, profitpercentage, charges, power, storage, investmentType: 'Crypto Mining'});
    await investmentplans.save()
    req.flash('success', 'Successfully added a new mining package.')
    res.redirect('/admin/admin.investmentplans');
});
...
router.put('/admin/admin.investmentplans/:id', isAdminLoggedIn, onlyAdmin, async(req, res) => {
    ...
    req.flash('success', 'Successfully edited mining package')
    res.redirect('/admin/admin.investmentplans')
});
...
router.delete('/admin/admin.investmentplans/:id', isAdminLoggedIn, onlyAdmin, async (req, res) => {
    ...
    req.flash('success', 'Successfully deleted mining package.')
    res.redirect('/admin/admin.investmentplans')
});
